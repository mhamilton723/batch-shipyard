...
class ApiContext(sy.CliContext):
    ...
    def stream_file(self, id, task_id, cluster_id, filename="stderr.txt"):
        if 'job_specifications' not in self.config:
            self.config["job_specifications"] = [{"id": None}]
        if id is not None:
            self.config["job_specifications"][0]["id"] = id
        if cluster_id is not None:
            self.config["pool_specification"]["id"] = cluster_id
        cf.initialize(self.config)
        cf.action_data_stream(
            self.batch_client,
            self.config,
            "{},{},{}".format(id, task_id, filename),
            False)
...
class Pool(object):
    ...
    def stream_file(self, id, task_id, filename="stderr.txt"):
        self.api_context.stream_file(id, task_id, self.id, filename)
